...
**Typescript** 는 동적 타입언어인**JavaScript** 를 타입이 있는 형태로 코딩할 수 있게 해주는 언어입니다. 

따라서 TypeScript로 짜여진 코드를 JavaScript로 변환해 줄 트랜스파일러(컴파일러)가 필요합니다. 컴파일은 다음과 같은 커맨드로 사용 가능합니다.

```jsx
tsc foo.ts
```

해당 컴파일 과정에 여러 옵션을 정할 수 있는데 이 옵션을 작성하는 파일이 tsconfig.json이다.
...
```jsx
{
	...
	"paths": {"@src/*": ["src/*"]}, /* 'baseUrl'를 기준으로 불러올 모듈의 위치를 재지정하는 엔트리 시리즈 */
